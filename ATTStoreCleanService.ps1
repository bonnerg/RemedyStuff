...
#                                                                             #
#                                                                             #
#     File name     ATTStoreCleanService.ps1                                  #
#                                                                             #
#   Org File name   PSService.ps1                                             #
#                                                                             #
...
#  																			  #
#    2019-07-16  GB Modified orginal sample service for use as a cleaner for  #
#                   the BMC ARS MidTier ATTStore directory that can fill up   #
#                   when users 'X' out of thier browser rather than           #
#                   gracefully logging out. Naturally this can caues problems #
#                   from filling up the disk potentualy causing Tomcat to     #
#                   crash to mearly not allowing user to upload any more      #
#                   attachments.                                              #
###############################################################################
#Requires -version 2
...
$scriptVersion = "2019-07-15"

$attpath = "C:\Program Files\BMC Software\ARSystem\midtier\attstore"
$period = 3600 # seconds = 1 Hour, how often does the service wake up and execute.
...
$serviceDisplayName = "BMC Remedy Midtier attstore cleaner"
$ServiceDescription = "PowerShell service script to clean BMC Remedy Midtier attstore in accordance with KA354125"
...
if ($Service) {                 # Run the service
  ...
  try {
    ...
    do { # Keep running until told to exit by the -Stop handler
	  ...
	  if (-not (Test-Path $attpath)) #If the path does not exist, write an error log entry and exit
		{
			Write-EventLog -LogName $logName -Source $serviceName -EventId 1010 -EntryType Error -Message "$scriptName Directory $attpath not found" 
			Send-PipeMessage $pipeName "exit"
		} # else if the path does exist then run our cleanup routine
	  Get-ChildItem -path $attpath | where {$_.LastWritetime -lt(date).addhours(-1)} | remove-item
        }
